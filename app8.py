...
def image():
    if request.method == 'POST':
        ...
        high_pass = cv2.Laplacian(luminosity_gray, cv2.CV_64F)
        cv2.imwrite('static/high_pass_filtered.jpg', high_pass)
        # 3. Image sharpening

        # Unsharp masking
        gaussian = cv2.GaussianBlur(high_pass, (9, 9), 10.0)
        unsharp_image = cv2.addWeighted(high_pass, 1.5, gaussian, -0.5, 0)
