...
def image():
    if request.method == 'POST':
        ...
        _, gaussian_threshold = cv2.threshold(sobel_x.astype("uint8"), 127, 255, cv2.THRESH_BINARY)
        ...
        ret, thresh = cv2.threshold(gaussian_threshold, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        # Noise removal
