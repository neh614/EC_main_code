...
def image():
    if request.method == 'POST':
        ...
        for contour in contours:
            ...
            # decide grade only, do not count tumor/lymph/metastasis
            ...
            # drawing contours only for internal grading visualization (not displayed now)
            cv2.drawContours(gray_for_contour, [contour], -1, (0, 255, 0), 2)

        if total_cells > 0:
            average_grade_score = grade_score / total_cells
        ...
        # NOTE: we are NOT saving contour_overlay.jpg anymore
        # cv2.imwrite(str(STATIC_DIR / 'contour_overlay.jpg'), gray_for_contour)

        # ---------------- CNN PREDICTION + STAGE ----------------
        ...
        # --------- IMAGES TO DISPLAY (NO CONTOURS IMAGE) ---------
