...
def image():
    if request.method == 'POST':
        ...
        # ---------- 2. GRADING USING SHAPE FEATURES ---------- #
        ...
        if total_regions > 0:
            # majority voting for final grade
            if high_count >= low_count and high_count >= intermediate_count:
                grade = "High Grade (Disorganized/Solid Growth)"
            elif low_count >= intermediate_count:
