...
def image():
    if request.method == 'POST':
        ...
        high_pass = cv2.Laplacian(luminosity_gray.astype("uint8"), cv2.CV_64F)
        cv2.imwrite('static/high_pass_filtered.jpg', high_pass)
        # 3. Image sharpening


        # Laplacian filter for sharpening
        laplacian = laplace(high_pass)
        ...
        adaptive_threshold = cv2.adaptiveThreshold(laplacian_image.astype("uint8"), 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
        ...
        ret, thresh = cv2.threshold(adaptive_threshold.astype("uint8"), 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        # Noise removal
...
##        def predict_and_display_image(model, img_path, class_labels):
        ...
        if class_result[0]== 0:
            str_label = "Endometrial Adocarcinoma"
            status1="stage1 cancer"
            print("The predicted image of the Endometrial Adocarcinoma is with a accuracy of {} %".format(result[class_result[0]]*100))
            accuracy="The predicted image of the Endometrial Adocarcinoma is with a accuracy of {}%".format(result[class_result[0]]*100)
           
           
            
        elif class_result[0]== 1:
            str_label  = "Endometrial hyper plsia"
            status1="stage2 cancer"
            print("The predicted image of the Endometrial hyper plsia is with a accuracy of {} %".format(result[class_result[0]]*100))
            accuracy="The predicted image of the Endometrial hyper plsia is with a accuracy of {}%".format(result[class_result[0]]*100)
