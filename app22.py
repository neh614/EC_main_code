...
def image():
    if request.method == 'POST':
        ...
        laplacian = laplace( gaussian_filtered.astype("uint8"))
        laplacian_image =  gaussian_filtered - laplacian * 255
        cv2.imwrite('static/laplacian_sharpened.jpg', laplacian_image)

        # 4. Thresholding

        # Gaussian thresholding
        _, gaussian_threshold = cv2.threshold(laplacian_image, 127, 255, cv2.THRESH_BINARY)
